...
import { transcribeAndUnderstand } from './asr/index';
...
app.post('/va/resolve', async (req, reply) => {
  ...
  const nlu = await transcribeAndUnderstand(audio, 'ar-SY');
...
app.post('/va/issue', async (req, reply) => {
  ...
  await supa.from('ops_tasks').insert({ type: handler, payload: { session_id, meta, route } });
  ...
  fast.get('/va/realtime', { websocket: true }, (conn) => {
    conn.socket.on('message', async (msg: Buffer) => {
      try {
        ...
        conn.socket.send(JSON.stringify({ partial: text, session_id }));
      } catch (e) {
        conn.socket.send(JSON.stringify({ error: String(e) }));
      }
    });
  });
});
...
app.listen({ port: cfg.port, host: '0.0.0.0' }).then(() => {
  app.log.info(`VA server listening on ${cfg.port}`);
});
